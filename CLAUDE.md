...
<!-- claude-only -->
- Reusable skills: `.claude/skills/` (in Claude Code, also reachable through the matching file in `.claude/commands/`)
<!-- /claude-only -->
<!-- codex-only
...
<!-- claude-only -->
## Operating in Cowork (read this if you are Cowork, not a CLI)

Most people here work in Cowork; the handful on a CLI are admins. Cowork will
probably not have these skills installed as Skills, so when this folder is your
working folder, read `.claude/skills/<name>/SKILL.md` and do what it says. Match
the request against the table above, follow that file, and put `acme-brand` on
top of whatever it produces. Nobody will type a skill name or a slash command,
so work out the intent yourself. Read this file at the start of each session,
and write the marker described above.
<!-- /claude-only -->
<!-- codex-only
...
`.claude/skills/` directly: ask to be added to `config.yaml` `skill_owners`,
